...
現在の実装は、実装計画の第 1-4 段階の一部を含む MVP である。OpenAI embedding は環境変数がある場合だけ検索 ranking に加わり、未設定または失敗時は FTS5 と metadata に戻る。週次 scheduler、file watcher、Android push 通知、認証は後続段階で追加する。
...
| `OPENAI_API_KEY` | 任意 | 設定すると OpenAI embedding を検索 ranking に使う |
